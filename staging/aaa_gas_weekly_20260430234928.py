"""AAA Gas Weekly threshold pipeline (KXAAAGASW family) — rev 2.

Family signature: aaa:gas:weekly_threshold

This is a revision of /Users/wilsonw/mm-setup/auto_theo/staging/aaa_gas_weekly_20260430201923.py.
The prior staged pipeline used a stationary 1-day σ of 0.013 USD/gal multiplied by
sqrt(horizon_days). Backtest on 36 KXAAAGASD events showed max-decile-deviation =
37.7pp (worst-decile predicted=1.0 vs actual=0.62). Root cause: stationary σ is
calibrated to a single reference theo (KXAAAGASD-26APR29) at one point in a
rally regime, and is too tight when the realized regime is rallying or chopping.

The fix carried in this rev: compute σ from the EIA WEEKLY archive
(archive/aaa/national_regular_eia_weekly/<YYYY-MM-DD>.json — 1857 files spanning
1990-2026) by taking the sample stdev of weekly log-returns over a rolling
window of SIGMA_ROLLING_WEEKS observations, then converting to a horizon σ via
σ_h = (σ_weekly / sqrt(7)) * sqrt(horizon_days). A floor at 0.5 × stationary
prevents over-confidence in calm regimes; a cap at 0.10 USD/gal prevents
pathological blowup.

The post-mortem on the FIRST attempt: an earlier rev tried rolling σ from
archive/aaa/national_regular/<date>.json (daily files). Those files are
EIA-weekly forward-filled to daily — day-over-day log-returns are mostly
exactly 0, so realized σ collapsed and the pipeline became MORE confident
(calibration went from 37.7pp to ~58pp). σ source is now WRONG. This file
fixes it by reading the underlying EIA weekly source directly.
...
# Upper bound on σ. Per spec.vol_estimation_hints.method_suggestion ceiling
# ("σ ≤ 0.10"). The current rally regime realized 7-day move is 27c, i.e.
# ≈10x the stationary 1d σ. We let σ inflate via realized weekly vol but
# ceiling it at 10c rather than quote insanely wide.
SIGMA_CAP_USD = 0.10
...
class AAAGasWeeklyPipeline(Pipeline):
    ...
    def _compute_horizon_sigma(self, horizon_days: float, now: datetime) -> tuple[float, str]:
        ...
        - Applies floor (0.5 × stationary 1-day σ) and cap (0.10 USD/gal).
        """
