...
class TruflationEvDailyPipeline(Pipeline):
    ...
    def _live_kalshi_strikes_for_event(self, event_ticker: str) -> Optional[list[float]]:
        """Return the live Kalshi strike grid for `event_ticker`, or None.

        Per spec instructions: when real markets are initialized by Kalshi the
        actual grid may differ. Pipeline must dispatch on event ticker and emit
        ...
        For now the live discovery looks at the archive of Kalshi markets
        (auto_theo/archive/kalshi/markets/) by event-ticker prefix. If that
        archive is empty for this event, we fall back to the canonical 15-strike
        grid (which is what the spec already documents). When 0 markets are
        found AND the spec says no canonical grid, we return [] so build_theos
        can return None gracefully.
        """
        ...
        # No archive — fall back to canonical grid (from spec).
